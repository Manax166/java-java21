...
public class Optional_01_Test {
    ...
    public void test_optional_ifPresent() throws Exception {
        ...
        Optional<Person> optPerson = persons.stream().filter( x -> x.getAge() == 18)
        .findFirst();
    ...
    public void test_optional_notPresent() throws Exception {
        ...
        // TODO utiliser la méthode "findFirst"
        Optional<Person> optPerson = Optional.ofNullable(persons.stream()
                .filter(x -> x.getAge() == 75)
                .findFirst()
                .orElseThrow(NotPresentException::new));
